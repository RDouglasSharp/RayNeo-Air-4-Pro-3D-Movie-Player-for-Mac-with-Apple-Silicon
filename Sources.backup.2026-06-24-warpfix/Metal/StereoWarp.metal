...
// Bilinear sample from read texture (kernels can't use .sample() with access::read)
float4 bilinearSampleTex(texture2d<float, access::read> tex, float tw, float th, float u, float v) {
    ...
    float4 a = tex.read(uint2(x0, y0));
    float4 b = tex.read(uint2(x1, y0));
    float4 c = tex.read(uint2(x0, y1));
    float4 d = tex.read(uint2(x1, y1));
...
kernel void stereoWarp(
    ...
    // CVPixelBuffer row 0 = top, but Metal texture y=0 = bottom — flip Y when sampling
    ...
    float leftFlipY = params.inputHeight - srcY;
    float rightFlipY = params.inputHeight - srcY;
    float4 leftPixel = bilinearSampleTex(sourceTexture, params.inputWidth, params.inputHeight, leftSrcX, leftFlipY);
    float4 rightPixel = bilinearSampleTex(sourceTexture, params.inputWidth, params.inputHeight, rightSrcX, rightFlipY);
        ...
        for (int dx = -16; dx <= 16; dx++) {
            for (int dy = -16; dy <= 16; dy++) {
                ...
                if (checkPos.x >= 0 && checkPos.x < (int)params.outputWidth &&
                    ...
                    float4 check = eyeTexture.read(as_type<uint2>(checkPos));
                    if (check.a >= 0.01f) {
                        nearest = check;
                        found = 1;
                        break;
