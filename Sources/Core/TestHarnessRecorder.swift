...
public class TestHarnessRecorder {
    ...
    public func startRecording(width: Int, height: Int, fps: Double) throws {
        ...
        let targetBitrate = Int(
            Double(max(20, Int(Double(width) * Double(height) * fps * 0.4)))
        )
        let keyInterval = Int(fps)
        var propsDict = [String: Any]()
        propsDict["AVVideoAverageBitrateKey"] = targetBitrate
        propsDict[AVVideoMaxKeyFrameIntervalKey] = keyInterval
        propsDict[AVVideoProfileLevelKey] = AVVideoProfileLevelH264HighAutoLevel
        var settingsDict = [String: Any]()
        settingsDict[AVVideoCodecKey] = AVVideoCodecType.h264
        settingsDict[AVVideoWidthKey] = width
        settingsDict[AVVideoHeightKey] = height
        settingsDict[AVVideoCompressionPropertiesKey] = propsDict
        let videoSettings = settingsDict
    ...
    public func finish(
        ...
        print("Avg latency: \(String(format: "%.2f", report.avgTotalMs))ms")
        print("p95 latency: \(String(format: "%.2f", report.p95Ms))ms")
        print("Max latency: \(String(format: "%.2f", report.maxMs))ms")
