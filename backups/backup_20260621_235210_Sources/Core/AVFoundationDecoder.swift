...
public class AVFoundationDecoder: VideoDecoderProtocol {
    ...
    public func loadVideo(at url: URL) throws {
        ...
        // Get frame rate
        if let tracks = videoTrack as NSObject as? AVAssetTrack,
           let scale = tracks.tracks(withMediaType: .video).first?.trackTimeScale {
            frameRate = Double(scale)
        }
    ...
    public func decodeNextFrame() throws -> CVBuffer? {
        guard
            let videoTrackOutput = videoTrackOutput,
            !videoTrackOutput.isFinished
        else {
            return nil
        }
