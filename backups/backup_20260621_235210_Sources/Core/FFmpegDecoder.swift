...
let AV_NOPTS_VALUE = Int64(0x8000000000000000)
let AVERROR_EOF = -12
...
public class FFmpegDecoder: VideoDecoderProtocol {
    ...
    public func loadVideo(at url: URL) throws {
        ...
        for i in 0..<nbStreams {
            ...
            if let codecpar = av_stream_get_codecpar(stream) {
    ...
    private func convertToPixelBuffer(frame avFrame: OpaquePointer) -> CVBuffer? {
        ...
        srcPtrs.pointee = srcData[0]
        srcPtrs.advanced(by: 1).pointee = srcData[1]
        srcPtrs.advanced(by: 2).pointee = srcData[2]
        ...
        var dstPtr = UnsafeMutablePointer<UInt8>(OpaquePointer(dstData))
        let dstPtrs: [UnsafeMutablePointer<UInt8>?] = [dstPtr, nil, nil, nil]
        let dstLinesize = CInt(dstStride)
        let dstStrides: [CInt] = [dstLinesize, 0, 0, 0]

        let rows = dstPtrs.withUnsafeBufferPointer { dstPtrBuf in
            dstStrides.withUnsafeBufferPointer { dstStrideBuf in
                sws_scale(
                    ctx,
                    srcPtrs,
                    srcStrides,
                    0,
                    CInt(h),
                    dstPtrBuf.baseAddress!.withMemoryRebound(to: UnsafePointer<UnsafeMutablePointer<UInt8>?>.self, capacity: 1) { $0 },
                    dstStrideBuf.baseAddress!.withMemoryRebound(to: UnsafePointer<CInt>.self, capacity: 1) { $0 }
                )
            }
        }
