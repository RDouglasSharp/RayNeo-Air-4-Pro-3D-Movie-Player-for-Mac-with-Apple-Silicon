...
public class VideoPlayer: NSObject, ObservableObject, VideoPlayerProtocol {
    private let logger = Logger(subcategory: "VideoPlayer")
    ...
    func stepFrame() {
        ...
        do {
            if let frame = try decoder.decodeNextFrame() as? CVPixelBuffer {
                currentFrame = frame
            }
        } catch {
            logger.error("Step frame failed: \(error.localizedDescription)")
        }
    }
    ...
    private func decodeLoop() async {
        ...
        while !Task.isCancelled {
            ...
            do {
                guard let buffer = try decoder.decodeNextFrame() as? CVPixelBuffer else { break }
                
                let presentationTime = CMTimeGetSeconds(buffer.metadata?.value(forKey: "presentationTime") as? CMTime ?? CMTime.zero)
