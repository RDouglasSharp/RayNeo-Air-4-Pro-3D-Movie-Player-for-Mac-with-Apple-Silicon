...
public class MetalPipeline {
    ...
    public func createTexture(
        ...
        desc.textureUsage = [.shaderRead]
        desc.pixelFormat = pixelFormat
        desc.width = width
        desc.height = height
        desc.storageMode = .private

        guard let texture = device.makeTexture(descriptor: desc, options: [], slice: nil, row: 0) else {
        ...
        if let srcBase = CVPixelBufferGetBaseAddress(pixelBuffer) {
            ...
            let dstBytesPerRow = texture.rowsPerImage * pixelFormat.pixelByteCount
    ...
    public func createTexture(fromPixelBuffer pixelBuffer: CVPixelBuffer) -> MTLTexture {
        ...
        case kCVPixelFormatType_16Grey:
    ...
    public func getTextureInfo(_ texture: MTLTexture) -> TextureInfo {
        ...
        info.rowsBytes = texture.rowsPerImage * 4
        return info
    }
    ...
    public func packageAll(
        ...
        let depthTexture = createTexture(fromPixelBuffer: depthMap, pixelFormat: .rFloat)
